...
  // 1. Derive the published ads list immediately when 'ads' changes
...
  const nextAd = () => {
    // Use the derived 'publishedAds' list
...
  // Ensure the current index is valid whenever publishedAds changes (e.g., if the ad list shrinks)
  useEffect(() => {
...
  // Log the filtered ads for debugging if needed
  // console.log('Published Ads:', publishedAds);

  // Determine the current ad to display from the filtered list
...
                <motion.div
                  key={currentAd._id} // Use ad ID as key for smoother transitions
...
                  />
                  <div className="mt-4">
                    <h3 className="text-xl mb-2">{currentAd.title}</h3>
...
                    {publishedAds.map((_, idx) => ( // Iterate over the filtered list
                      <button
                        key={idx}
