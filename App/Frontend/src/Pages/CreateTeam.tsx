...
import { createTeam, TeamMember } from '../store/slices/teamSlice';
...
export default function CreateTeam() {
  ...
  const changeRole = (memberId: string, newRole: 'Leader' | 'Editor' | 'Viewer') => {
                  ...
                  {members.map((member) => (
                    <div key={member.id} className="flex items-center justify-between p-3 border rounded-lg bg-muted/30">
                      ...
                      <div className="flex items-center gap-2">
                        <Select 
                          value={member.role} 
                          onValueChange={(value) => changeRole(member.id, value as 'Leader' | 'Editor' | 'Viewer')}
                        >
